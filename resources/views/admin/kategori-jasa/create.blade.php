...
<div class="admin-section">
    ...
    <form action="{{ route('admin.kategori-jasa.store') }}" method="POST" class="admin-card p-6 space-y-5">
        @csrf

        <div>
            <label class="admin-label">Nama Kategori *</label>
            <input type="text" name="nama" class="admin-input" value="{{ old('nama') }}" placeholder="Contoh: Rias Pengantin" required>
        </div>
