    ...
    <div class="min-h-screen bg-slate-50/50 flex flex-col lg:flex-row"
        ...
        <aside class="hidden lg:flex w-64 flex-col bg-white border-r border-border min-h-screen sticky top-0">
            ...
            <div class="p-6 border-b border-border">
                ...
                <p class="text-sm font-medium truncate text-gray-700" title="{{ $user->nama }}">{{ $user->nama }}</p>
            </div>
                                ...
                                <ul class="divide-y divide-border">
                                    @foreach ($pendingBookings->take(5) as $b)
                                        <li class="py-4 flex items-center justify-between gap-4">
                                            <div>
                                                <p class="font-semibold text-gray-900">{{ $b->user->nama }} <span class="text-muted-foreground text-xs font-normal">· {{ $b->kode_pemesanan ?? $b->id }}</span></p>
                                                <p class="text-xs text-muted-foreground mt-1">{{ $b->jenis }} · {{ $b->tanggal_pakai ?? $b->tanggal_pemesanan }} · {{ $b->lokasi }}</p>
                                            </div>
                                ...
                                <tbody class="divide-y divide-border">
                                    @foreach ($bookings as $b)
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            <td class="py-3 pr-3">
                                                <p class="font-semibold text-gray-900">{{ $b->user->nama }}</p>
                                                <p class="text-xs text-muted-foreground">{{ $b->kode_pemesanan ?? $b->id }}</p>
                                            </td>
                                            <td class="py-3 pr-3 text-gray-700">{{ $b->jenis }}</td>
                                            <td class="py-3 pr-3 text-gray-600">{{ $b->tanggal_pakai ?? $b->tanggal_pemesanan }}</td>
                                            <td class="py-3 pr-3 text-gray-600">{{ $b->lokasi }}</td>
                                            <td class="py-3 pr-3">
                        ...
                        <div class="mt-5 grid gap-4 md:grid-cols-2">
                            @foreach ($testimonials as $t)
                                <div class="rounded-2xl border border-border p-5 shadow-sm hover:shadow-md transition">
                                    <p class="font-semibold text-gray-900">{{ $t->user->nama }}</p>
                                    ...
                                    <p class="mt-3 text-sm text-gray-600 italic">"{{ $t->isi_testimoni }}"</p>
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                @else
                    ...
                    <div x-show="tab === 'beranda'" class="space-y-6">
                        ...
                        <div class="rounded-3xl bg-gradient-warm border border-border p-8 shadow-soft animate-scale-in">
                            ...
                            <h2 class="mt-2 font-serif text-3xl font-bold text-gray-900">{{ $user->nama }} ✨</h2>
                            <p class="mt-2 text-sm text-gray-600">Pantau status booking Bunda di sini.</p>
                            {{-- TODO: ganti route('dashboard') dengan route('booking') setelah halaman booking dibuat --}}
                            <a href="{{ route('dashboard') }}" class="mt-5 inline-flex items-center justify-center rounded-full px-6 py-2.5 bg-primary text-primary-foreground font-semibold hover:bg-rose-700 active:scale-95 shadow-sm transition">
                                Buat Booking Baru
                            </a>
                        </div>
                        ...
                        <div class="rounded-3xl bg-white border border-border p-6 shadow-soft animate-scale-in">
                            ...
                            @if ($bookings->isEmpty())
                                {{-- TODO: ganti route('dashboard') dengan route('booking') setelah halaman booking dibuat --}}
                                <p class="text-sm text-muted-foreground mt-4">Belum ada booking. <a href="{{ route('dashboard') }}" class="text-primary font-medium hover:underline">Mulai booking pertama</a>.</p>
                            @else
                                <ul class="mt-3 divide-y divide-border">
                                    @foreach ($bookings->take(3) as $b)
                                        <li class="py-4 flex items-center justify-between">
                                            <div>
                                                <p class="font-semibold text-gray-900">{{ $b->jenis }}</p>
                                                <p class="text-xs text-muted-foreground mt-1">{{ $b->tanggal_pakai ?? $b->tanggal_pemesanan }} · {{ $b->lokasi }}</p>
                                            </div>
                                            <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-medium" :class="statusClass('{{ $b->status }}')">
                                                {{ $b->status }}
                                            </span>
                            ...
                            <div class="mt-5 grid gap-3">
                                @foreach ($bookings as $b)
                                    <div class="rounded-2xl border border-border p-5 flex flex-wrap items-center justify-between gap-3 shadow-sm hover:shadow-md transition">
                                        <div>
                                            <p class="font-semibold text-gray-900">{{ $b->jenis }} <span class="text-xs text-muted-foreground font-normal">· {{ $b->kode_pemesanan ?? $b->id }}</span></p>
                                            <p class="text-xs text-gray-600 mt-1">{{ $b->tanggal_pakai ?? $b->tanggal_pemesanan }} · {{ $b->lokasi }}</p>
                                        </div>
                                        <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-medium" :class="statusClass('{{ $b->status }}')">
                                            {{ $b->status }}
                                        </span>
                    ...
                    <div x-show="tab === 'new'" class="rounded-3xl bg-white border border-border p-8 text-center shadow-soft animate-scale-in">
                        ...
                        {{-- TODO: ganti route('dashboard') dengan route('booking') setelah halaman booking dibuat --}}
                        <a href="{{ route('dashboard') }}" class="mt-6 inline-flex items-center justify-center rounded-full px-6 py-2.5 bg-primary text-primary-foreground font-semibold hover:bg-rose-700 active:scale-95 shadow-sm transition">
                            Buka Form Booking
                        </a>
                    </div>
                    ...
                    <div x-show="tab === 'profile'" class="rounded-3xl bg-white border border-border p-8 shadow-soft max-w-xl animate-scale-in">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="font-serif text-2xl font-semibold text-gray-900">{{ $user->nama }}</p>
                        ...
                        <dl class="mt-6 space-y-4 text-sm">
                            ...
                            @if ($user->no_hp)
                                <div class="flex items-start justify-between gap-4 border-b border-border/50 pb-2">
                                    ...
                                    <dd class="text-gray-900 font-semibold text-right">{{ $user->no_hp }}</dd>
                                </div>
                            @endif
                            <div class="flex items-start justify-between gap-4 border-b border-border/50 pb-2">
                                <dt class="text-gray-500 font-medium">Peran</dt>
                                <dd class="text-gray-900 font-semibold text-right capitalize">{{ $user->getRoleNames()->first() ?? '-' }}</dd>
                            </div>
                        </dl>
                    </div>
            ...
            <div class="bg-white rounded-3xl p-8 max-w-lg w-full shadow-petal border border-border animate-scale-in"
                ...
                <dl class="mt-5 space-y-3.5 text-sm max-h-[60vh] overflow-y-auto pr-1">
                    <div class="flex items-start justify-between gap-4 border-b border-border/50 pb-2">
                        ...
                        <dd class="text-gray-900 font-semibold text-right" x-text="detail.user?.nama"></dd>
                    </div>
                    <div class="flex items-start justify-between gap-4 border-b border-border/50 pb-2">
                        <dt class="text-gray-500 font-medium">WhatsApp</dt>
                        <dd class="text-gray-900 font-semibold text-right" x-text="detail.no_hp"></dd>
                    </div>
                    <div class="flex items-start justify-between gap-4 border-b border-border/50 pb-2">
                        <dt class="text-gray-500 font-medium">Layanan</dt>
                        <dd class="text-gray-900 font-semibold text-right" x-text="detail.jenis"></dd>
                    </div>
                    <div class="flex items-start justify-between gap-4 border-b border-border/50 pb-2">
                        <dt class="text-gray-500 font-medium">Tanggal</dt>
                        <dd class="text-gray-900 font-semibold text-right" x-text="detail.tanggal_pakai || detail.tanggal_pemesanan"></dd>
                    </div>
                    <div class="flex items-start justify-between gap-4 border-b border-border/50 pb-2">
                        <dt class="text-gray-500 font-medium">Lokasi</dt>
                        <dd class="text-gray-900 font-semibold text-right" x-text="detail.lokasi"></dd>
                    </div>
