    ...
    <div class="space-y-3">
        @forelse($pesanans as $p)
            @php
                ...
                $badge = $statusMap[$p->status] ?? [
                    'label' => ucfirst($p->status),
                    'class' => 'bg-[#E2D4C0] text-[#4A2E28]',
                ];
            @endphp
            <a href="{{ route('user.pemesanan.show', $p->id) }}"
                ...
                <div class="flex items-start gap-4 min-w-0">
                    {{-- Ikon jenis --}}
                    <div
                        class="shrink-0 mt-0.5 flex h-10 w-10 items-center justify-center rounded-xl bg-[#FAF3E0] border border-[#E2D4C0]">
                    ...
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2 mb-1">
                            <span
                                class="font-semibold text-[#4A0F1A] text-sm">{{ $p->kode_pemesanan ?? '#' . $p->id }}</span>
                            <span
                                class="inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-semibold {{ $badge['class'] }}">
                                {{ $badge['label'] }}
                            </span>
                        </div>
                ...
                <div class="shrink-0 text-right">
                    <p class="font-serif font-semibold text-[#C8960C]">Rp {{ number_format($p->total_harga, 0, ',', '.') }}
                    </p>
                    <p class="text-[10px] text-[#4A2E28]/50 mt-1">{{ $p->created_at->format('d M Y') }}</p>
                    <i data-lucide="chevron-right"
                        class="h-4 w-4 text-[#4A2E28]/40 mt-1 ml-auto group-hover:text-[#C8960C] transition"></i>
                </div>
            </a>
        @empty
