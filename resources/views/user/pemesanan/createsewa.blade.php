...
<div x-data="orderForm()" x-init="initForm" class="max-w-4xl mx-auto space-y-6 my-10 animate-fade-in px-4 sm:px-0">
    ...
    <div class="mb-8 border-b border-gray-200 pb-4">
        <p class="text-xs font-bold uppercase tracking-[0.2em] text-[#D4AF37]">BUAT PESANAN SEWA</p>
        <h2 class="text-3xl font-bold text-[#800000]">Formulir Sewa Barang</h2>
    </div>
    ...
    <form action="{{ route('user.pemesanan.store') }}" method="POST" class="bg-white rounded-2xl border border-gray-100 p-6 md:p-8 shadow-sm">
        ...
        <div x-show="step === 1" x-transition.opacity>
            ...
            <div class="bg-[#FAF3E0] p-4 rounded-lg flex justify-between items-center">
                <span class="text-sm font-medium text-[#800000]">Durasi: <span class="font-bold" x-text="durationDays + ' hari'"></span></span>
                <span class="text-sm font-medium text-[#800000]">Subtotal: <span class="font-bold text-lg">Rp <span x-text="formatRupiah(subtotal)"></span></span></span>
            </div>
        ...
        <div x-show="step === 2" x-transition.opacity style="display: none;">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <label class="p-4 border rounded-xl cursor-pointer" :class="deliveryMethod === 'ambil_sendiri' ? 'border-[#800000] bg-[#FAF3E0]/30' : ''">
                    <input type="radio" name="metode_pengambilan" value="ambil_sendiri" x-model="deliveryMethod"> <span class="font-bold">Ambil Sendiri</span>
                </label>
                <label class="p-4 border rounded-xl cursor-pointer" :class="deliveryMethod === 'dikirim' ? 'border-[#800000] bg-[#FAF3E0]/30' : ''">
                    <input type="radio" name="metode_pengambilan" value="dikirim" x-model="deliveryMethod"> <span class="font-bold">Dikirim</span>
                </label>
            </div>
            ...
            <div x-show="deliveryMethod === 'dikirim'" class="space-y-4 bg-gray-50 p-4 rounded-xl border">
                ...
                <div class="pt-2 border-t">
                    ...
                    <div class="flex gap-4 mt-2">
                        <label><input type="radio" name="metode_pengembalian" value="antar_sendiri" checked> Antar Sendiri</label>
                        <label><input type="radio" name="metode_pengembalian" value="dijemput"> Dijemput (+Biaya)</label>
                    </div>
                </div>
            </div>
        ...
        <div x-show="step === 3" x-transition.opacity style="display: none;">
            <h3 class="text-xl font-bold text-[#800000] mb-6 border-b pb-2">STEP 3: Ringkasan</h3>
            <div class="bg-gray-50 rounded-xl border p-5 mb-6">
                <div class="flex justify-between mb-2"><span>Biaya Sewa</span><span class="font-bold">Rp <span x-text="formatRupiah(subtotal)"></span></span></div>
                <div class="flex justify-between mb-2" x-show="ongkosKirim > 0"><span>Biaya Kirim</span><span class="font-bold">Rp <span x-text="formatRupiah(ongkosKirim)"></span></span></div>
            ...
            <div class="space-y-4 mb-6">
                <label class="block p-4 border rounded-xl cursor-pointer" :class="paymentMethod === 'dp' ? 'border-[#800000] bg-[#FAF3E0]/30' : ''">
                    <input type="radio" name="metode_bayar" value="dp" x-model="paymentMethod"> <span class="font-bold">Uang Muka (DP 50%)</span>
                </label>
                <label class="block p-4 border rounded-xl cursor-pointer" :class="paymentMethod === 'lunas' ? 'border-[#800000] bg-[#FAF3E0]/30' : ''">
                    <input type="radio" name="metode_bayar" value="lunas" x-model="paymentMethod"> <span class="font-bold">Lunas Langsung</span>
                </label>
            </div>
...
<script>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('orderForm', () => ({
            ...
            get ongkosKirim() { return (this.deliveryMethod === 'dikirim' && this.selectedZonaId) ? parseFloat(this.zonaLokasis.find(z => z.id == this.selectedZonaId)?.tarif || 0) : 0; },
