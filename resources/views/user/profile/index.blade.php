...
<div x-data="{ isEditing: false, showSecurity: false }">
    ...
    <div class="rounded-2xl border border-[#E2D4C0] bg-white shadow-[0_2px_8px_rgba(74,15,26,0.06)] overflow-hidden">
        <div class="grid lg:grid-cols-[240px_1fr]">
            ...
            {{-- Sisi kiri — Avatar --}}
            <div class="flex flex-col items-center justify-start border-b border-[#E2D4C0] bg-[#FAF3E0]/70 px-6 py-8 lg:border-b-0 lg:border-r">
                ...
                <div class="h-24 w-24 rounded-full overflow-hidden border-2 border-[#C8960C]/40 bg-[#4A0F1A] shadow-sm">
                    ...
                    @else
                        <div class="h-full w-full flex items-center justify-center text-[#FAF3E0] text-3xl font-serif font-light">
                ...
                {{-- Upload foto --}}
                <form action="{{ route('user.profile.photo.update') }}" method="POST" enctype="multipart/form-data" class="mt-5">
                    @csrf
                    @method('PUT')
            ...
            <div class="px-8 py-8">
                ...
                <form action="{{ route('user.profile.update') }}" method="POST">
                    ...
                    <div class="space-y-5">
                        {{-- Nama --}}
                        <div>
                            <label class="block text-[10px] uppercase tracking-wider text-[#4A2E28]/50 mb-2">Nama Lengkap</label>
                            <input type="text" name="nama" value="{{ $u?->nama ?? $u?->name ?? '' }}"
                                   :disabled="!isEditing"
                        ...
                        <div>
                            ...
                            <input type="email" name="email" value="{{ $u?->email ?? '' }}" readonly
                        ...
                        {{-- No HP --}}
                        <div>
                            <label class="block text-[10px] uppercase tracking-wider text-[#4A2E28]/50 mb-2">Nomor Telepon</label>
                ...
                <div class="mt-8 border-t border-[#E2D4C0] pt-6">
                    ...
                    <div x-show="isEditing && showSecurity" x-cloak>
                        <form action="{{ route('user.profile.update') }}" method="POST" class="space-y-4">
                            ...
                            <div>
                                <label class="block text-[10px] uppercase tracking-wider text-[#4A2E28]/50 mb-2">Password Saat Ini</label>
                            ...
                            <div>
                                <label class="block text-[10px] uppercase tracking-wider text-[#4A2E28]/50 mb-2">Password Baru</label>
                                <input type="password" name="password"
                                       class="w-full border-b border-[#E2D4C0] pb-2 bg-transparent outline-none transition text-sm font-semibold text-[#4A0F1A] focus:border-[#C8960C]">
                            </div>
                            <div>
                                <label class="block text-[10px] uppercase tracking-wider text-[#4A2E28]/50 mb-2">Konfirmasi Password Baru</label>
