...
@section('content')
    <div class="max-w-3xl mx-auto space-y-5 print:max-w-none print:space-y-0">
        ...
        {{-- Lembar Invoice --}}
        <div id="invoice-card"
            class="rounded-3xl bg-white border border-[#E2D4C0] shadow-[0_4px_24px_rgba(74,15,26,0.07)] overflow-hidden print:rounded-none print:border-0 print:shadow-none">
            ...
            <div class="p-8 sm:p-10">
                ...
                {{-- Header --}}
                <div
                    class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 pb-7 mb-7 border-b border-[#E2D4C0]">
                    <div>
                        ...
                        <p class="text-xs text-[#4A2E28]/60 mt-1.5">
                            Tanggal: <span
                                class="font-medium text-[#4A2E28]">{{ $pesanan->created_at->format('d M Y') }}</span>
                        </p>
                    </div>
                ...
                <div class="grid sm:grid-cols-2 gap-6 mb-8">
                    <div class="rounded-2xl border border-[#E2D4C0] bg-[#FAF3E0] px-5 py-4 space-y-1">
                        <p class="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#C8960C] mb-2">Ditagihkan
                            Kepada</p>
                        ...
                        <p class="text-xs text-[#4A2E28]/60 leading-relaxed pt-0.5">
                            {{ $pesanan->lokasi ?? 'Diambil langsung ke store' }}</p>
                    </div>
                    <div class="rounded-2xl border border-[#E2D4C0] bg-[#FAF3E0] px-5 py-4 space-y-1">
                        <p class="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#C8960C] mb-2">Detail
                            Pelaksanaan</p>
                    ...
                    <table class="min-w-full text-sm text-left">
                        <thead>
                            <tr class="bg-[#FAF3E0] border-b border-[#E2D4C0]">
                                <th class="px-5 py-3 text-[10px] font-bold uppercase tracking-[0.08em] text-[#4A2E28]">Nama
                                    Produk / Layanan</th>
                                <th
                                    class="px-5 py-3 text-[10px] font-bold uppercase tracking-[0.08em] text-[#4A2E28] text-center w-24">
                                    Jml</th>
                                <th
                                    class="px-5 py-3 text-[10px] font-bold uppercase tracking-[0.08em] text-[#4A2E28] text-right w-36">
                                    Harga Satuan</th>
                                <th
                                    class="px-5 py-3 text-[10px] font-bold uppercase tracking-[0.08em] text-[#4A2E28] text-right w-36">
                                    Subtotal</th>
                        ...
                        <tbody class="divide-y divide-[#E2D4C0] bg-white">
                            ...
                            @foreach ($pesanan->detailPemesanans as $detail)
                                ...
                                <tr>
                                    ...
                                    <td class="px-5 py-4 text-right font-mono text-[#4A2E28]">Rp
                                        {{ number_format($detail->harga, 0, ',', '.') }}</td>
                                    <td class="px-5 py-4 text-right font-semibold font-mono text-[#4A0F1A]">Rp
                ...
                <div class="flex flex-col items-end gap-2 max-w-sm ml-auto">
                    <div class="flex justify-between w-full text-sm text-[#4A2E28]/70">
                        ...
                        <span class="font-mono font-medium text-[#4A2E28]">Rp
                            {{ number_format($total_item_cost, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between w-full text-sm text-[#4A2E28]/70">
                        <span>Ongkos Lokasi</span>
                        <span class="font-mono font-medium text-[#4A2E28]">Rp
                            {{ number_format($pesanan->ongkos_lokasi ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div
                        class="flex justify-between w-full text-base font-bold border-t border-dashed border-[#E2D4C0] pt-3 mt-1">
                    ...
                    <div class="mt-6 rounded-2xl border border-[#E2D4C0] bg-[#FAF3E0] px-5 py-4">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between text-[#4A2E28]/70">
                                ...
                                <span class="font-mono font-medium text-[#4A2E28]">Rp
                                    {{ number_format($bayar->jumlah_bayar, 0, ',', '.') }}</span>
                            </div>
                            @if ($bayar->tahap !== 'langsung' && $sisa > 0)
                                <div class="flex justify-between text-[#4A2E28]/70">
                                    <span>Sisa Pelunasan</span>
                                    <span class="font-mono font-semibold text-[#4A0F1A]">Rp
                            ...
                            <div class="flex items-center gap-2 pt-1">
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[10px] font-semibold {{ $badgeClass }}">
    ...
    <script>
        ...
        window.downloadPNG = function() {
            const pdfUrl = '{{ route("user.pemesanan.invoice.pdf", $pesanan->id) }}';
            ...
            pdfjsLib.getDocument(pdfUrl).promise
                ...
                .then(function(canvas) {
                    ...
                    link.download = 'invoice-{{ $pesanan->kode_pemesanan }}.png';
