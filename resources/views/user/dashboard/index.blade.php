...
<div class="mb-4 rounded-2xl border border-[#E2D4C0] bg-white shadow-[0_2px_8px_rgba(74,15,26,0.06)] p-6">
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
        @foreach ([
            ['label' => 'Menunggu Konfirmasi', 'count' => $statusCounts['konfirmasi'],  'numColor' => 'text-amber-600', 'bg' => 'bg-amber-50',  'border' => 'border-amber-200'],
            ['label' => 'Menunggu Pembayaran', 'count' => $statusCounts['pembayaran'],  'numColor' => 'text-blue-600',  'bg' => 'bg-blue-50',   'border' => 'border-blue-200'],
            ['label' => 'Berlangsung',          'count' => $statusCounts['berlangsung'], 'numColor' => 'text-green-700', 'bg' => 'bg-green-50',  'border' => 'border-green-200'],
            ['label' => 'Selesai',              'count' => $statusCounts['selesai'],     'numColor' => 'text-[#4A0F1A]', 'bg' => 'bg-[#FAF3E0]', 'border' => 'border-[#E2D4C0]'],
        ] as $stat)
            <div class="rounded-xl border {{ $stat['border'] }} {{ $stat['bg'] }} p-4 text-center">
                ...
                <p class="mt-1.5 text-[9px] font-semibold uppercase tracking-[0.15em] text-[#4A2E28] leading-tight">{{ $stat['label'] }}</p>
            </div>
        @endforeach
    </div>
</div>
...
@else
    <div class="mb-4 flex items-center gap-3 rounded-2xl border border-[#E2D4C0] bg-white shadow-[0_2px_8px_rgba(74,15,26,0.06)] px-5 py-4">
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <a href="{{ route('public.katalog.index') }}"
       class="group flex items-center gap-4 rounded-2xl border border-[#E2D4C0] bg-white shadow-[0_2px_8px_rgba(74,15,26,0.06)] p-5 hover:border-[#C8960C]/60 hover:shadow-md transition">
        <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#4A0F1A] text-[#FAF3E0] group-hover:bg-[#7B1C2E] transition">
        ...
        <i data-lucide="arrow-right" class="h-4 w-4 text-[#C8960C]/60 ml-auto group-hover:text-[#C8960C] transition"></i>
    </a>
    <a href="{{ route('public.katalog.index') }}"
       class="group flex items-center gap-4 rounded-2xl border border-[#E2D4C0] bg-white shadow-[0_2px_8px_rgba(74,15,26,0.06)] p-5 hover:border-[#C8960C]/60 hover:shadow-md transition">
        <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#4A0F1A] text-[#FAF3E0] group-hover:bg-[#7B1C2E] transition">
