...
<div class="space-y-8 animate-scale-in">
    ...
    {{-- 1. HEADER UTAMA: Title & Identitas Akun Login Dinamis --}}
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        <div class="rounded-3xl bg-white border border-maroon-deep/5 p-6 shadow-sm flex flex-col justify-between min-h-[140px] transition-all hover:shadow-md">
            ...
            <p class="text-3xl font-bold text-maroon-deep tracking-tight">
                {{ $pemesanan->whereBetween('created_at',[now()->startOfMonth(), now()->endOfMonth()])->count() }}
            </p>
        </div>
        ...
        <div class="rounded-3xl bg-white border border-maroon-deep/5 p-6 shadow-sm flex flex-col justify-between min-h-[140px] transition-all hover:shadow-md">
            <div class="flex items-center justify-between">
                <p class="text-xs font-bold uppercase tracking-wider text-emerald-600">Dikonfirmasi / Lunas</p>
                <div class="h-8 w-8 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center">
                    <i data-lucide="check-circle" class="h-4 w-4"></i>
                </div>
            </div>
            <p class="text-3xl font-bold text-maroon-deep tracking-tight">
                {{ $pemesanan->whereIn('status',['disetujui', 'lunas'])->count() }}
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="rounded-3xl bg-white border border-maroon-deep/5 p-6 shadow-sm space-y-4">
            ...
            <div class="flex flex-col gap-2.5">
                <a href="{{ route('user.pemesanan.pilihjenis') }}" class="w-full inline-flex items-center gap-2 rounded-full bg-maroon-deep px-5 py-3 text-sm text-cream font-medium transition hover:opacity-90 shadow-sm">
                    <i data-lucide="plus-circle" class="h-4 w-4 text-gold-soft"></i> Buat Pemesanan Baru
                </a>
                <a href="{{ route('user.pemesanan.index') }}" class="w-full inline-flex items-center gap-2 rounded-full border border-maroon-deep/10 bg-cream/20 px-5 py-3 text-sm text-maroon-deep font-medium transition hover:bg-cream hover:border-maroon-deep/30">
                    <i data-lucide="eye" class="h-4 w-4 text-gold"></i> Riwayat Pemesanan
                </a>
        ...
        <div class="rounded-3xl bg-white border border-maroon-deep/5 p-6 shadow-sm lg:col-span-2">
            <div class="flex items-center justify-between border-b border-neutral-100 pb-3 mb-4">
                ...
                <a href="{{ route('user.pemesanan.index') }}" class="text-xs font-semibold text-gold hover:underline transition">Lihat semua</a>
            </div>
            ...
            <div class="overflow-x-auto rounded-2xl border border-neutral-100">
                <table class="min-w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-neutral-50 text-maroon-deep/90">
                        ...
                        <tr class="hover:bg-cream/10 transition-colors cursor-pointer" onclick="window.location='{{ route('user.pemesanan.show', $p->id) }}'">
                            <td class="px-5 py-3.5 font-semibold text-maroon-deep">
                                {{ $p->kode_pemesanan ?? 'ORD-'.$p->id }}
                            </td>
                            <td class="px-5 py-3.5">
                                <span class="text-xs font-bold uppercase px-2 py-0.5 rounded {{ $p->kategori_order == 'acara' ? 'bg-blue-50 text-blue-700' : 'bg-purple-50 text-purple-700' }}">
                                    {{ $p->kategori_order }}
                                </span>
                            </td>
                            <td class="px-5 py-3.5 font-light text-neutral-600">
                                {{ $p->tanggal_pakai ? \Carbon\Carbon::parse($p->tanggal_pakai)->format('d M Y') : '-' }}
                            </td>
                            <td class="px-5 py-3.5 text-center">
                                @php
                                    $statusColor = 'bg-gray-100 text-gray-700';
                                    if($p->status == 'pending') $statusColor = 'bg-amber-50 text-amber-700 border border-amber-100';
                                    if(in_array($p->status, ['disetujui', 'lunas'])) $statusColor = 'bg-green-50 text-green-700 border border-green-100';
                                    if($p->status == 'batal') $statusColor = 'bg-rose-50 text-rose-700 border border-rose-100';
                                @endphp
                                <span class="inline-flex rounded-full px-3 py-1 text-xs font-medium {{ $statusColor }}">
                                    {{ ucfirst($p->status) }}
                                </span>
                            </td>
                        </tr>
